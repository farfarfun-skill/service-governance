...
| Production | Versioned formal package installed from the artifact registry/index | Build -> publish -> install (or upgrade/rollback to) an exact version from the registry -> `<cli> server start` |

`start`/`stop`/`restart`/`run` never take a `dev`/`prod` argument and never branch on one. The installed package alone determines what runs; a host only ever has one active install at a time, and whoever installed it (a developer running the local build, or ops installing a pinned registry version) already knows which one it is. The steps genuinely asymmetric between the two modes by name are `install-dev` (always the local-build flavor), `install-prod` (always the first-time registry-install flavor, for a host that has nothing installed yet), and `publish` (always the release-to-registry flavor). `upgrade [version]`/`rollback <version>` operate on whatever is already installed regardless of which mode put it there — in practice they only matter on a prod host, since a dev host just reruns `install-dev`.
...
2. Structure that CLI as a subcommand tree with two groups:
   - A `server` group that owns runtime lifecycle only: `<cli> server start` (detaches and backgrounds itself), `<cli> server run` (identical startup, but stays attached to the invoking terminal instead of detaching — the foreground counterpart of `start`), plus `server stop`/`server status`, so the CLI's own subcommands can back a `scripts/setup.sh` dispatch (see [Bash Service Guide](../bash-service-guide/SKILL.md)) instead of the script reimplementing lifecycle logic itself.
   - Top-level self-management subcommands that own the CLI's own package lifecycle: `<cli> upgrade [version]` (install the latest, or an explicit, version via the ecosystem's package manager — this is what a lifecycle script's `upgrade [version]` action should call), `<cli> rollback <version>` (install an explicit older version — this is how you roll back, not by reusing an install action; a lifecycle script's `rollback <version>` action should call this the same way), and `<cli> uninstall` (stop the running server first if one is live, then remove the package). Do not add a `<cli> install` subcommand: the CLI cannot install itself before it exists, so the very first install always goes through `pip install`/`npm install -g`/`funbuild install` directly — this direct path is exactly what a lifecycle script's `install-prod [version]` action wraps.
...
4. Own PID lifecycle entirely inside the CLI, not the Bash wrapper: `<cli> server start` and `<cli> server run` write a `<cli-name>.pid` file into the same directory as whichever config path was actually resolved in step 3 (default path or `--config` override) — e.g. `~/.config/farfarfun/funflix/funflix.pid` next to `~/.config/farfarfun/funflix/config.toml`. `<cli> server status` locates the process by reading that same file. `<cli> server stop`'s actual termination step should go through `funshell port <port> --kill` against the port `server start` bound to, rather than hand-signaling the PID read from that file — a Python-based CLI should depend on the `funshell` package directly and call its port-kill API in-process; a non-Python CLI (npm, etc.) shells out to the `funshell port <port> --kill` command instead. This keeps the CLI fully self-sufficient for `start`/`run`/`stop`/`status` even on a host with no `scripts/setup.sh` or repository checkout at all, and removes any need for a Bash-owned `.run/<service>.pid` file for services built to this contract.
5. Support discrete flags for the parameters operators change most often (`--port`, and similarly named flags for host/workers/etc.), and make an explicit flag override the same key from `--config` when both are given. This lets a start command rely on a config file while still allowing a one-off override without editing or duplicating that file.
6. `start`/`run` in the lifecycle script must call exactly `<installed-cli> server start [--config <path>] [--port ...]` (backgrounded, invoked by the Bash `start` action) or `<installed-cli> server run [--config <path>] [--port ...]` (foreground, invoked via `exec` by the Bash `run` action) against whatever package is currently installed — never a source-tree script, a raw `node`/`python -m` invocation bypassing the CLI, or a dev server left running outside the lifecycle script's control. Both command shapes are identical for a locally built install and a registry-pinned install; only the config path and, implicitly, which package got installed differ.
7. `<cli> server status` must derive running/stopped state from the same PID file that `server start`/`server run` write, and report the installed package's version (e.g. via `pip show`/`npm ls -g`/the CLI's own `--version`) alongside PID/port, so operators can confirm what is actually running without a separate lookup.
...
6. Create a clean production-like environment, install the exact version from the selected repository, and confirm resolution did not fall back to a local path, `node_modules` symlink, or unintended index/registry. A lifecycle script's `install-prod [version]` action implements this step directly via the ecosystem's package manager when the host has nothing installed yet — omit the version to install whatever is currently published without disturbing an existing install; pass one to pin exactly that version.
...
Rolling back a bad production release is the same shape of flow with an older, explicit version. A lifecycle script's `rollback <version>` action (see [Bash Service Guide](../bash-service-guide/SKILL.md)) implements exactly this: it calls `<installed-cli> rollback <version>` when the CLI supports it, or otherwise reinstalls the exact older pinned version directly through the ecosystem's package manager. `<version>` is mandatory — there is no "latest" rollback, and `rollback` never reuses `install-prod`'s no-op-if-already-installed behavior; it always forces the target version.
