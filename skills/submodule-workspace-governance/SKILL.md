...
- In `setup.sh`, `all` means different things for different actions: for service actions (`start`/`stop`/`restart`/`run`/`status`/`install-dev`/`install-prod`/`upgrade`/`rollback`/`publish`) it scopes to CLI-bearing apps only (`api` + `web`); for `build` it scopes to every app under `apps/`, including core-library and plugin apps. Never conflate the two.
...
- `scripts/init.sh`, `scripts/build.sh`, and `scripts/setup.sh` all exist; `scripts/setup.sh --help`/usage output covers `install-dev`/`install-prod`/`upgrade`/`rollback`/`publish` and `start`/`stop`/`restart`/`run`/`status` for every CLI-bearing app plus `build` for every submodule.
