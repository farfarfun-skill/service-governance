...
3. Add cross-repo scripts under `scripts/`.
   ...
   - `setup.sh` (required, alongside `init.sh`/`build.sh`) — the cross-repo `<action> <target>` dispatcher, covering service lifecycle (`start`/`stop`/`restart`/`run`/`status`) for service apps, release lifecycle (`install-dev`/`install-prod`/`upgrade`/`rollback`/`publish`) for service + package apps, and `build` for service + package apps (nested-workspace apps excluded from all three). Every `<product>-dev` repo ships this from the start, same as `init.sh`/`build.sh` — it is not something to add later once a service app shows up. See "Design scripts/setup.sh" below.
...
- `<action>` splits into three groups with different `all` semantics — spell this out in the script's usage text, since it is the single most common source of confusion in this pattern:
  ...
  - **Release actions** (`install-dev`, `install-prod`, `upgrade`, `rollback`, `publish`): apply to every service app and every package app — a core-library or plugin app still needs these even though it has no service actions.
  - **`build`**: same scope as release actions — every service app and every package app. `all` means every submodule in `.gitmodules` except nested-workspace apps. Finish with one `funbuild push` at the dev-repo level regardless of how many apps were built, so the pointer bump lands as a single commit.
  - Nested-workspace apps are excluded from all three groups — bump their pointer like any other submodule, but dispatch into them (if needed) by `cd`-ing into the nested workspace and running its own `scripts/setup.sh`, not through this script's aliases.
...
- `scripts/init.sh`, `scripts/build.sh`, and `scripts/setup.sh` all exist; `scripts/setup.sh --help`/usage output covers `start`/`stop`/`restart`/`run`/`status` for every service app, `install-dev`/`install-prod`/`upgrade`/`rollback`/`publish` for every service + package app, and `build` for every service + package app, with nested-workspace apps excluded from all three.
